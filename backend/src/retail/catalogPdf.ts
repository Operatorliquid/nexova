...
const embedLogo = async (doc: PDFDocument, logoUrl?: string | null) => {
  ...
  try {
    ...
    console.warn("[CatalogPDF] No se pudo cargar el logo:", (error as any)?.message || error);
    return null;
  }
};
...
const buildCatalogPdfBuffer = async (params: CatalogPdfParams) => {
  ...
  const drawProduct = (product: CatalogPdfProduct) => {
    ...
    const blockHeight =
      titleLines.length * 14 + (descLines.length ? descLines.length * 13 + 4 : 0) + 6;
