...
export async function checkOpenAIConnectivity(): Promise<ServiceCheckResult> {
  if (!openaiClient) {
    return {
      ok: false,
...
export type AvailableSlot = {
  startISO: string;   // fecha/hora en ISO (UTC)
  humanLabel: string; // texto lindo para mostrar al paciente
};

export type PatientProfileUpdates = {
  name?: string | null;
...
type AgentActionBase = {
  profileUpdates?: PatientProfileUpdates | null;
};

export type AgentAction =
  | ({ type: "NONE" } & AgentActionBase)
  | ({ type: "ASK_CLARIFICATION"; reply: string } & AgentActionBase)
  | ({
...
export type AgentResult = {
  ...
  action: AgentAction;
  pendingSlotHint?: PendingSlotHint;
  profileUpdates?: PatientProfileUpdates | null;
};
...
export async function runWhatsappAgent(
  ...
  if (
    ctx.patientProfile.pendingSlotISO &&
  ...
  if (/(gracias|listo|ok|perfecto|bárbaro|genial)/.test(lower)) {
    return {
      replyToPatient:
        "De nada 🙌. Si necesitás reprogramar o pedir otro turno, escribime por acá.",
      action: { type: "NONE" },
    };
  }
...
async function runDomainAgent(
  ...
  let execution: AgentExecutionResult | null = null;
  ...
  const profileUpdates =
    execution.profileUpdates ?? execution.action.profileUpdates ?? null;
  ...
  if (action.type === "offer_slots") {
    return {
      ...
      pendingSlotHint: action.slots?.[0]
        ? {
  ...
  if (action.type === "confirm_slot") {
    ...
    if (!slotToConfirm && ctx.patientProfile.pendingSlotISO) {
      slotToConfirm = {
        ...
        humanLabel: ctx.patientProfile.pendingSlotHumanLabel ?? "el turno pendiente",
      };
    }

    const slotISO =
      slotToConfirm?.startISO || ctx.patientProfile.pendingSlotISO || null;

    const intent = analyzePatientIntent(ctx.text);
    if (intent.rejected && !intent.confirmed) {
      return {
        replyToPatient:
          "Entendido, no confirmo ese turno. Decime qué día y horario querés para ofrecerte opciones correctas.",
    ...
    return {
      replyToPatient:
        action.reply ||
        `Perfecto, confirmo el turno ${
          slotToConfirm?.humanLabel || ctx.patientProfile.pendingSlotHumanLabel || ""
        }.`,
      action: {
        type: "CREATE_APPOINTMENT",
        dateTimeISO: slotISO,
        reason:
          normalizeReasonInput(action.reason) ||
          normalizeReasonInput(ctx.patientProfile.pendingSlotReason) ||
 ...
 */
function simpleHeuristicAgent(ctx: WhatsappAgentContext): AgentResult {
  const text = ctx.text;
  const lower = text.toLowerCase();
  ...
  const doctorRecentlyOfferedSlot = lastDoctorMessage
    ? /turno|horario disponible|reservado/.test(
        lastDoctorMessage.text.toLowerCase()
      )
    : false;
  ...
  if (asksPrice) {
    ...
    if (normal && emergency) {
      return {
        replyToPatient:
          `La consulta tiene un valor de $ ${normal.toLocaleString("es-AR")} ` +
    ...
    return {
      replyToPatient:
        "Todavía no tengo cargado el valor de la consulta. Escribí \"menu\" para ver las opciones y coordinar tu turno.",
      action: { type: "NONE" },
    };
  }
  ...
  if (asksSchedule) {
    if (profile.officeDays || profile.officeHours) {
      ...
      const where =
        profile.clinicName || profile.officeAddress
    ...
    return {
      replyToPatient:
        "No tengo cargados los días y horarios exactos del consultorio. Escribí \"menu\" para ver las opciones y coordinar tu turno.",
      action: { type: "NONE" },
    };
  }
  ...
  if (asksAddress) {
    ...
    return {
      replyToPatient:
        "No tengo cargada la dirección exacta. Escribí \"menu\" para ver las opciones y coordinar un turno.",
      action: { type: "NONE" },
    };
  }
  ...
  if (asksContact) {
    ...
    return {
      replyToPatient:
        "No tengo un teléfono cargado, pero podés escribirme \"menu\" para ver cómo sacar, reprogramar o cancelar un turno.",
      action: { type: "NONE" },
    };
  }
  ...
  if (asksSpecialty) {
    ...
    return {
      replyToPatient:
        `No tengo más datos sobre la especialidad. Escribí "menu" para ver las opciones y coordinar un turno.`,
      action: { type: "NONE" },
    };
  }
  ...
  if (mentionsAppointment && ctx.patientProfile.needsName) {
    return {
      replyToPatient:
        "Genial, pero antes necesito tu nombre y apellido completos.",
      action: { type: "NONE" },
    };
  }
  ...
  if (mentionsAppointment && ctx.patientProfile.needsBirthDate) {
    return {
      replyToPatient:
        "¿Me pasás tu fecha de nacimiento? Podés escribirla como 31/12/1990.",
      action: { type: "NONE" },
    };
  }
  ...
  if (mentionsAppointment && ctx.patientProfile.needsAddress) {
    return {
      replyToPatient:
        "Necesito tu dirección (calle y número) para terminar de registrar la ficha.",
      action: { type: "NONE" },
    };
  }
  ...
  if (mentionsAppointment && ctx.patientProfile.needsInsurance && ctx.businessType === "HEALTH") {
    return {
      replyToPatient:
        "¿Tenés obra social o prepaga? Decime el nombre exacto así lo anoto.",
      action: { type: "NONE" },
    };
  }
  ...
  if (mentionsAppointment && ctx.patientProfile.needsConsultReason) {
    return {
      replyToPatient:
        "¿Cuál es el motivo principal de la consulta? (ej: control anual, dolor de cabeza, etc.)",
      action: { type: "NONE" },
    };
  }
  ...
  // Buscar hoy / mañana
  let dayOffset = 0;
  if (lower.includes("pasado mañana")) dayOffset = 2;
  else if (lower.includes("mañana")) dayOffset = 1;
  ...
  const confirmAppointment = (slot: AvailableSlot): AgentResult => {
  ...
  if (acceptance) {
    if (doctorRecentlyOfferedSlot) {
      ...
      return {
        replyToPatient:
          "Intenté reservar ese turno pero ya no está disponible. Decime otro horario y lo coordinamos.",
        action: { type: "NONE" },
      };
    }
    return {
      replyToPatient:
        "Genial. Para avanzar contame qué día u horario te viene bien y te propongo un turno disponible.",
      action: { type: "NONE" },
    };
  }
  ...
  if (!mentionsAppointment) {
    return {
      replyToPatient:
        'No puedo ayudarte con eso ahora mismo. Escribí "menu" para ver las opciones para sacar, reprogramar o cancelar un turno.',
      action: { type: "NONE" },
    };
  }
  ...
  if (!chosenSlot) {
    return {
      replyToPatient:
        "Por ahora no veo turnos libres en los próximos días. Probá escribirme de nuevo más tarde o llamá a la recepción de la clínica 🙏.",
      action: { type: "NONE" },
    };
  }
...
function normalizeReasonInput(value?: string | null) {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (
    /^(si|sí|dale|ok|okay|listo|me sirve|confirmo|perfecto|vale|está bien)/i.test(
      trimmed
    )
  ) {
    return null;
...
function analyzePatientIntent(text: string) {
  if (!text) {
    return { confirmed: false, rejected: false };
  }
  const lower = text.toLowerCase();
  const confirmedRegex =
    /(me sirve|lo tomo|confirmo|dale|perfecto|agendalo|agéndalo|sí,? ese|si ese|queda ese|ok ese|está bien|esta bien|lo confirmo|reservalo|reserválo|listo ese|genial,? gracias|de una|aseguralo)/;
  const rejectedRegex =
    /(no quiero|no ese|ese no|prefiero otro|otro horario|wtf|qué decís|que decís|no me sirve|no confirmes|cambiemos|cambiarlo|busca otro|buscá otro|ninguno|no,? gracias)/;
  const confirmed = confirmedRegex.test(lower);
  const rejected = rejectedRegex.test(lower);
  return { confirmed, rejected };
}

function findSlotMatchingMessage(
  ctx: WhatsappAgentContext,
  text: string
): AvailableSlot | null {
  ...
  let preference = parsePreferenceFromText(text) || ({} as ParsedPreference);

  const preferredDayDate =
    ctx.patientProfile.preferredDayISO &&
    !Number.isNaN(Date.parse(ctx.patientProfile.preferredDayISO))
      ? new Date(ctx.patientProfile.preferredDayISO)
      : null;

  const pendingSlotDate =
    ctx.patientProfile.pendingSlotISO &&
  ...
  const scored = ctx.availableSlots
    .map((slot) => {
      ...
      const baseScore = scoreSlotAgainstPreference(
        slotDate,
        preference,
        now,
        ctx.timezone
      ...
      const anchorPenalty =
        anchorMatters && anchorDate
          ? Math.abs(
              startOfDay(slotDate, ctx.timezone).getTime() -
                startOfDay(anchorDate, ctx.timezone).getTime()
            ) / 86400000
          : 0;
      return {
        slot,
...
function formatSlotLabelFromISO(iso: string, timezone?: string) {
  try {
