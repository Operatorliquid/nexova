...
async function runRetailAgent(ctx, openai) {
    ...
    try {
        ...
        const activePromotions = ctx.activePromotions || ctx.promotions;
        const storeProfile = ctx.storeProfile || ctx.businessProfile || {};
        const businessAlias = storeProfile.businessAlias ||
            ctx.businessAlias ||
            null;
        const incomingMedia = ctx.incomingMedia || ctx.media;
        const retailState = ctx.retailConversationState || ctx.conversationState;
...
function buildAgentPrompt(ctx, catalogText, promosText, storeText, mediaText, retailState) {
    var _a, _b, _c;
...
function sanitizeAction(action) {
    ...
    cleaned.items = cleaned.items
        ...
        return {
            ...
            normalizedName: typeof it.normalizedName === "string" && it.normalizedName.trim()
    ...
    if (typeof cleaned.mode !== "string")
        cleaned.mode = "merge";
    if (cleaned.mode !== "merge" && cleaned.mode !== "replace")
        cleaned.mode = "merge";
    if (typeof cleaned.status !== "string")
        cleaned.status = "pending";
    if (!["pending", "confirmed", "cancelled"].includes(cleaned.status))
        cleaned.status = "pending";
    ...
    if (cleaned.paymentProof && typeof cleaned.paymentProof === "object") {
        ...
        cleaned.paymentProof = {
            ...
            amount: Number.isFinite(pp.amount) ? Number(pp.amount) : (_a = pp.amount) !== null && _a !== void 0 ? _a : null,
    ...
    if (typeof cleaned.intent !== "string")
        cleaned.intent = "";
    if (!Number.isFinite(cleaned.confidence))
        cleaned.confidence = 0.65;
    cleaned.confidence = Math.max(0, Math.min(1, Number(cleaned.confidence)));
    return cleaned;
}
function formatCatalogForPrompt(catalog) {
    if (!catalog)
        return "(sin catálogo cargado)";
    if (Array.isArray(catalog) && catalog.every((x) => typeof x === "string")) {
        return catalog.slice(0, 80).join(" · ");
    }
    if (Array.isArray(catalog) && catalog.length > 0 && typeof catalog[0] === "object") {
        const truncate = (txt, max = 120) => txt.length > max ? `${txt.slice(0, max - 1).trimEnd()}…` : txt;
        const categorySet = new Set();
        const tagSet = new Set();
        const items = catalog.slice(0, 80).map((p) => {
            var _a, _b;
            const name = p.name || p.title || p.productName;
            const price = (_b = (_a = p.price) !== null && _a !== void 0 ? _a : p.unitPrice) !== null && _b !== void 0 ? _b : null;
        ...
        const tagLine = tagSet.size > 0 ? `Etiquetas: ${Array.from(tagSet).slice(0, 40).join(" · ")}` : "Etiquetas: (no declaradas)";
        return `${catLine}\n${tagLine}\nProductos:\n${items.join("\n")}`;
    }
    return "(catálogo en formato desconocido)";
}
function formatPromosForPrompt(promos) {
    if (!promos)
        return "(sin promos cargadas)";
    if (Array.isArray(promos) && promos.length === 0)
        return "(sin promos activas)";
...
function formatStoreProfileForPrompt(profile) {
    if (!profile)
        return "(sin info del negocio)";
    if (typeof profile === "string")
        return profile;
    if (typeof profile !== "object")
        return "(info del negocio en formato desconocido)";
    const lines = [];
    if (profile.name)
    ...
    if (profile.hours)
        lines.push(`- Horarios: ${profile.hours}`);
    if (profile.delivery)
        lines.push(`- Envíos: ${profile.delivery}`);
    if (profile.paymentMethods) {
        lines.push(`- Pagos: ${Array.isArray(profile.paymentMethods)
            ? profile.paymentMethods.join(", ")
            : profile.paymentMethods}`);
    }
    if (profile.notes)
        lines.push(`- Notas: ${profile.notes}`);
    return lines.length ? lines.join("\n") : JSON.stringify(profile).slice(0, 800);
}
function formatMediaForPrompt(media) {
    var _a;
    if (!media)
        return "(sin media)";
    if (typeof media === "object") {
        ...
        const count = Number.isFinite(media.count) ? Number(media.count) : (_a = urls === null || urls === void 0 ? void 0 : urls.length) !== null && _a !== void 0 ? _a : 0;
        return `- count: ${count}
- urls: ${(urls === null || urls === void 0 ? void 0 : urls.join(", ")) || "(s/dato)"}
- contentTypes: ${(contentTypes === null || contentTypes === void 0 ? void 0 : contentTypes.join(", ")) || "(s/dato)"}`;
    }
